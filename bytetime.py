...
class ByteTimeManager:
    ...
    def start_task(self, index):
        if 0 <= index < len(self.tasks):
            if self.current_task is not None:
                self.stop_task()  # Stop the current task if any
    ...
    def draw_ui(self):
        ...
        for i, task in enumerate(self.tasks):
            ...
            task_str = f"{status_icon} {task['name']} ({task['duration']:.0f}s / {task['estimated_time']}s)"
    ...
    def run(self):
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                task = self.stdscr.getstr().decode('utf-8')
                self.stdscr.addstr(curses.LINES - 1, 0, "예상 소요 시간(초): ")
                try:
                    estimated_time = int(self.stdscr.getstr().decode('utf-8'))
                except ValueError:
                    self.stdscr.addstr(curses.LINES - 1, 0, "잘못된 입력입니다. 예상 소요 시간을 0으로 설정합니다.")
                    estimated_time = 0
                curses.noecho()
